...
import { Download, KeyRound, Loader2, Save, Sparkles, Trash2 } from 'lucide-react'
...
import { Field, Header, ToggleField } from './settings-workspace-shared'
import { normalizeBoardView, selectClassName, swap } from './settings-workspace-utils'
...
type TranscriptionSettingsPanelProps = {
  settings: AppSettings
  busy: boolean
  onSave(input: AppSettingsUpdateInput): void | Promise<void>
}

const presetTimestampIntervals: ReadonlyArray<TranscriptionTimestampInterval> = ['none', 'segment', 30, 60, 120, 300, 600, 1800]

export function AppSettingsPanel({ settings, busy, onSave }: AppSettingsPanelProps) {
...
export function ProjectSettingsPanel({ settings, busy, onSave }: ProjectSettingsPanelProps) {
...
export function AiSettingsPanel({ settings, busy, onSave }: AiSettingsPanelProps) {
