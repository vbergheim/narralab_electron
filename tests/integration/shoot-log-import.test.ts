...
describe('shoot log import', () => {
  ...
  it('imports the newer OPPTAKSLOGG workbook with category, function, quote and quality', async () => {
    ...
    try {
      ...
      expect(first?.shootDayPlace).toBe('Bergen')
      expect(first?.shootDayProduction).toBe('Barnesykehuset')
      expect(first?.shootDayDirector).toBe('Ina Lerner Grevstad')
      expect(first?.shootDayPhotographer).toBe('Vegard Lund Bergheim')
      expect(first?.shootDayParticipants).toBe('Hai Nguyen')
      expect(first?.shootDayFolderName).toBe('')
      expect(first?.shootDayFileName).toBe('')
      expect(first?.shootDayClipCount).toBe('')
      expect(first?.shootDayDescription).toContain('Opptaksdagen fulgte hverdagen på en travel sykehusavdeling')
      expect(first?.shootDayStrongestMaterial).toContain('Samtalen med pårørende i korridoren ble særlig sterk')
      expect(first?.shootDayFollowUp).toContain('Følge Hai videre inn i kveldsvakten')
